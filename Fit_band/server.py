...
def user():
    ...
    user = execute_select_query(query)
    on_publish("user", "success")

    return user

@add.route('/update', methods = ['PUT'])
def update_city():
    name = request.form.get('name')
    city = request.form.get('city')
    ...
    return f"{name} update successfully"

@add.route('/maxsteps', methods = ['GET'])
def max_steps():
    query = f"select * from health where steps = (select MAX(steps) from health);"
    maximum = execute_select_query(query)
    on_publish("max", "success")
    return maximum
